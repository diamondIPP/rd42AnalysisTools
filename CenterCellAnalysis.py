...
class CenterCellAnalysis:
	def __init__(self, trans_grid, numstrips, clustersize):
		...
		self.percent_from_dist = np.unique(np.floor(np.power(np.arange(0, 1.05, 0.05), 2) * 100 + 0.5).astype('int32'))
		self.percent_from_area_fraction = np.unique(np.floor(np.multiply(np.arange(0, 1.05, 0.05), 100) + 0.5).astype('int32'))
		...
		self.sat_adc_between_cut = {}
		self.nosat_adc_between_cut = {}
	...
	def DoPHPlots(self, p_array, cells='all', drawHistos=True, suffixop='', cut='', typ='adc', isFriend=False):
		...
		for p in p_array:
			if 0 <= p <= 100:
				...
				if p in self.in_central_polygon_region.keys() and p in self.out_central_polygon_region.keys():
					tempcut = cut if cut != '' else self.ph_cuts(varzkey)
					tempcutin = self.trans_grid.cuts_man.AndCuts([tempcut, self.in_central_polygon_region[p]])
					tempcutout = self.trans_grid.cuts_man.AndCuts([tempcut, self.out_central_polygon_region[p]])
					if drawHistos:
						hname = 'PH{n}_H_{t}_cells_in_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ) if not isFriend else 'PH{n}_H_buffer_{v}_{t}_cells_in_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, v=self.trans_grid.noise_friend_buffer)
					...
					hname = 'PH{n}_H_{t}_in_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ) if not isFriend else 'PH{n}_H_buffer_{v}_{t}_in_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, v=self.trans_grid.noise_friend_buffer)
					self.DoPHPlot(hname, varz, 'PH{n} highest chs [{t}]'.format(n=self.num_strips, t=typ.upper()), tempcutin, cells, typ, drawHistos)

					if drawHistos:
						hname = 'PH{n}_H_{t}_cells_out_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ) if not isFriend else 'PH{n}_H_buffer_{v}_{t}_cells_out_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, v=self.trans_grid.noise_friend_buffer)
						self.DoCellOverlayPlot(hname, varz, 'PH{n} highest chs [{t}]'.format(n=self.num_strips, t=typ.upper()), tempcutout, cells, typ=typ)
						self.trans_grid.DrawCentralArea(hname, p)
					hname = 'PH{n}_H_{t}_out_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ) if not isFriend else 'PH{n}_H_buffer_{v}_{t}_out_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, v=self.trans_grid.noise_friend_buffer)
	...
	def DoCenterRegionStudies(self, area_or_dist='area', cells='all', drawHistos=True, suffixop='', cuts='', typ='adc', do_sat=True, isFriend=False):
		self.SetCenterRegionStudies()
		percents = self.percent_from_area_fraction if area_or_dist == 'area' else self.percent_from_dist
		for percent in percents:
			self.trans_grid.CreateTCutGScaledPolygon(percent)
		self.trans_grid.cuts_man.CreateBetweenCentralPlygonRegionsCut()
		self.DoCenterRegionHistos(percents, cells, drawHistos, suffixop, cuts, typ, isFriend=isFriend)
		self.DrawPHInAndOutGraphs(percents, suffixop if suffixop != '' else self.suffix[cells], typ, isFriend=isFriend)
	...
	def DrawPHInAndOutGraphs(self, percents, suffix, typ='adc', isFriend=False):
		...
		yarrayin = np.array([self.trans_grid.histo['PH{n}_H{isf}{t}_in_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, isf='_' if not isFriend else '_buffer_' + str(self.trans_grid.noise_friend_buffer) + '_')].GetMean() for p in percents], 'float64')
		yarrayinerrs = np.array([self.trans_grid.histo['PH{n}_H{isf}{t}_in_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, isf='_' if not isFriend else '_buffer_' + str(self.trans_grid.noise_friend_buffer) + '_')].GetMeanError() for p in percents], 'float64')
		yarrayout = np.array([self.trans_grid.histo['PH{n}_H{isf}{t}_out_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, isf='_' if not isFriend else '_buffer_' + str(self.trans_grid.noise_friend_buffer) + '_')].GetMean() for p in percents], 'float64')
		yarrayouterrs = np.array([self.trans_grid.histo['PH{n}_H{isf}{t}_out_polygon_{p}_percent_{s}'.format(p=p, s=suffix, n=self.num_strips, t=typ, isf='_' if not isFriend else '_buffer_' + str(self.trans_grid.noise_friend_buffer) + '_')].GetMeanError() for p in percents], 'float64')
		...
		ingraphname = 'PH{n}_H{isf}{t}_in_polygon_Vs_percent_area_inside_{s}'.format(n=self.num_strips, t=typ, s=suffix, isf='_' if not isFriend else '_buffer_' + str(self.trans_grid.noise_friend_buffer) + '_')
		...
		outgraphname = 'PH{n}_H{isf}{t}_out_polygon_Vs_percent_area_inside_{s}'.format(n=self.num_strips, t=typ, s=suffix, isf='_' if not isFriend else '_buffer_' + str(self.trans_grid.noise_friend_buffer) + '_')
	...
	def DoCenterSaturationStudies(self, percents, cells='all', suffix='', cuts='', typ='adc'):
		suffixop = self.suffix[cells] if suffix == '' else suffix
		self.SetCenterRegionStudies()
		...
		ingraphrationame = 'Sat_events_ratio_in_polygon_Vs_percent_area_in_{s}'.format(s=suffixop)
		...
		outgraphrationame = 'Sat_events_ratio_out_polygon_Vs_percent_area_in_{s}'.format(s=suffixop)
	...
	def GetCutsFromCutManager(self):
		self.in_central_polygon_region = self.trans_grid.cuts_man.in_central_polygon_region
		self.out_central_polygon_region = self.trans_grid.cuts_man.out_central_polygon_region
		self.between_central_polygon_regions = self.trans_grid.cuts_man.between_central_polygon_regions
		self.SetCutsForSaturationRatio()

	def SetCutsForSaturationRatio(self):
		for p, cut in self.in_central_polygon_region.iteritems():
		...
		for p, cut in self.out_central_polygon_region.iteritems():
		...
		for p, cut in self.between_central_polygon_regions.iteritems():
			self.sat_adc_between_cut[p] = self.trans_grid.cuts_man.AndCuts([self.sat_evts_region_cut, cut])
			self.nosat_adc_between_cut[p] = self.trans_grid.cuts_man.AndCuts([self.not_sat_evts_region_cut, cut])

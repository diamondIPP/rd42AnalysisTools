...
class HexagonalCell3D(Cell3D):
	def __init__(self, col_num=0, row_num=0, height=0, run=0):
		Cell3D.__init__(self, col_num, row_num, sides=6, run=run)
		self.p = 1.0
		self.h = height
		self.cutg = None

	def CreateTCutG(self):
		tempx = np.add(self.xcenter, np.divide(np.array([-self.p, -2 * self.p, -self.p, self.p, 2 * self.p, self.p, -self.p], 'f8'), 3.0, dtype='f8'), dtype='f8')

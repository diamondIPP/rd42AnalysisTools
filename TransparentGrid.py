...
class TransparentGrid:
    def __init__(self, dir='', run=0, col_pitch=0):
        ...
        self.colorPalleteExec1 = ro.TExec('ex1','gStyle->SetPalette(54);gStyle->SetNumberContours(99)')
    ...
    def FindPickleValues(self, do_offset_plots=True, do_binning=False):
        ...
        :return:
        """
        print 'Running: FindPickleValues'
        ...
        for col_type in col_types:
            if col_type == 'even' or self.num_cols > 1:
                ...
                if self.row_cell_info_diamond['num_' + col_type] != 0:
                    ...
                    func = ro.TF1('box_fcn_' + col_type, '[0]*(TMath::Erf((x-([3]-{p}))/[1])+1)/2-[2]*(TMath::Erf((x-[3])/[4])+1)/2+[5]'.format(p=(self.row_cell_info_diamond['num_' + col_type] + 2 ) * self.row_cell_info_diamond['height']), miny, maxy)
                    func.SetNpx(10000)
                    ...
                    func.SetParLimits(1, 0.01 * self.row_cell_info_diamond['height'], 1 * self.row_cell_info_diamond['height'])
                    func.SetParLimits(2, abs(z1 - z2) / 100.0, 10.0 * abs(z1 - z2))
                    func.SetParLimits(3, y2 - 500, y2 + 500)
                    func.SetParLimits(4, 0.01 * self.row_cell_info_diamond['height'], 1 * self.row_cell_info_diamond['height'])
                    ...
                    self.row_cell_info_diamond['0_' + col_type] = fit_prof_proj_y.Parameter(3) - self.row_cell_info_diamond['height'] * (self.row_cell_info_diamond['num_' + col_type] + 1)
                    self.row_cell_info_diamond['up_' + col_type] = fit_prof_proj_y.Parameter(3) - self.row_cell_info_diamond['height']
                else:
                    # the number of cells on this type of column has not been stablished. It will be established by fitting
                    func = ro.TF1('box_fcn_' + col_type, '[0]*(TMath::Erf((x-([3]-([6]+2)*{p}))/[1])+1)/2-[2]*(TMath::Erf((x-[3])/[4])+1)/2+[5]'.format(p=self.row_cell_info_diamond['height']), miny, maxy)
                    func.SetNpx(10000)
                    zmin, zmax = self.histo['vertical_limits_profile_{t}_py'.format(t=col_type)].GetMinimum(), self.histo['vertical_limits_profile_{t}_py'.format(t=col_type)].GetMaximum()
                    y1bin, y2bin = self.histo['vertical_limits_profile_{t}_py'.format(t=col_type)].FindFirstBinAbove((zmin + zmax) / 2.0), self.histo['vertical_limits_profile_{t}_py'.format(t=col_type)].FindLastBinAbove((zmin + zmax) / 2.0) + 1
                    y1, y2 = self.histo['vertical_limits_profile_{t}_py'.format(t=col_type)].GetXaxis().GetBinCenter(y1bin), self.histo['vertical_limits_profile_{t}_py'.format(t=col_type)].GetXaxis().GetBinCenter(y2bin)
    ...
    def FindThresholdCutFromCells(self, var='clusterChargeN', typ='adc', xmin=0, xmax=4800, deltax=10, it=0, doFit2=False):
        ...
        if len(self.mean_ph_cell_dic[typ].keys()) > 0:
            ...
            deltax = self.histo['mean_ph_per_cell_' + typ].GetBinWidth(1)
            ...
            if doFit:
                ...
                if self.fits['mean_ph_per_cell_' + typ].Ndf() < 2 and itt < 20:
                    self.FindThresholdCutFromCells(var, typ, xmin, xmax, deltax * 0.8, itt + 1)
                elif self.fits['mean_ph_per_cell_' + typ].Ndf() > 0:
                    if 0.5 <= self.fits['mean_ph_per_cell_' + typ].Chi2() / self.fits['mean_ph_per_cell_' + typ].Ndf() < 0.9 and itt < 20:
                        self.FindThresholdCutFromCells(var, typ, xmin, xmax, deltax * 0.8, itt + 1)
        else:
            self.GetMeanPHPerCell(var, typ)
    ...
    def SelectByHistogramPercentile(self, hname, qvaldic, percentile=80):
        if hname in self.histo.keys():
            hist = self.histo[hname]
            if hist:
                ...
                for col in xrange(self.dia_cols.num_cols):
                    for row in xrange(self.dia_cols.cols[col].num_rows):
                        if qvaldic[col][row] >= q_threshold:
                            self.gridAreas.AddGoodAreas(col, row, self.dia_cols)
                        else:
                            self.gridAreas.AddBadAreas(col, row, self.dia_cols)
    ...
    def FitLanGaus(self, name, conv_steps=100, color=ro.kRed, xmin=-10000000, xmax=-10000000):
        ...
        temph = ro.TH1F(self.histo[name].Clone('htemph'))
        temph.SetBinContent(1, 0)
        halfmax = temph.GetMaximum() / 2.0
        binLHalfMax = temph.FindFirstBinAbove(halfmax)
        binRHalfMax = temph.FindLastBinAbove(halfmax)
        phLHalfMax, phRHalfMax = temph.GetBinLowEdge(binLHalfMax), temph.GetBinLowEdge(binRHalfMax) + temph.GetBinWidth(binRHalfMax)
        phFWHMCenter = (phRHalfMax + phLHalfMax) / 2.0
        temph.Delete()
        ...
        ro.gPad.Update()
        ps = AddLineToStats(self.canvas[name], 'stats', 'mystats2', 'Mean_{Fit}', fitmean)
        self.histo[name].SetStats(0)
        self.canvas[name].Modified()
        ps.Draw()
        ps2 = AddLineToStats(self.canvas[name], 'mystats2', 'mystats', 'FWHMCenter', phFWHMCenter)
        self.histo[name].SetStats(0)
        self.canvas[name].Modified()
        ps2.Draw()
        self.trash.append(ps2)
        ro.gPad.Update()
    ...
    def FitTwoGaus(self, name, num_sigma=2.5, iterations=2):
        if name in self.histo.keys():
            if name in self.canvas.keys():
                if self.canvas[name]:
                    self.canvas[name].cd()
            hbla = self.histo[name]
            xmean, xrms, maxc, xmp = hbla.GetMean(), hbla.GetRMS(), hbla.GetMaximum(), hbla.GetBinCenter(hbla.GetMaximumBin())
            xmin, xmax = xmean - num_sigma * xrms, xmean + num_sigma * xrms
            func = ro.TF1('f_2_gaus_' + name, 'gaus(0)+gaus(3)', xmean - (num_sigma + 1) * xrms, xmean + (num_sigma + 1) * xrms)
            func.SetNpx(1000)
            func.SetLineStyle(1)
            func.SetLineColor(ro.kRed)
            func.SetLineWidth(2)
            params = np.array((maxc / 2., xmean, xrms, maxc, xmp, xrms), dtype='f8')
            params_liminf = [0, hbla.GetBinLowEdge(hbla.GetXaxis().GetFirst()), 0, 0,  hbla.GetBinLowEdge(hbla.GetXaxis().GetFirst()), 0]
            params_limsup = [hbla.GetEntries(), hbla.GetBinLowEdge(hbla.GetXaxis().GetLast() + 1), 10, hbla.GetEntries(), hbla.GetBinLowEdge(hbla.GetXaxis().GetLast() + 1), 10]
            func.SetParameters(params)
            for it, (linf, lsup) in enumerate(zip(params_liminf, params_limsup)):
                func.SetParLimits(it, linf, lsup)
            for it in xrange(iterations):
